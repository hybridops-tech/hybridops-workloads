...
- For the normative on-prem deployment path:
  - runtime vault
  - GCP Secret Manager
  - `ExternalSecret`
...
- Overlay-specific deployments may extend realm config with additional identity providers and clients.
...
- The Stage 1 realm uses the `hyops-learn` public PKCE client and the `learn_member` / `learn_admin` realm roles.
